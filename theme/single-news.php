			...
			<div class="yb">
				...
				<div class="g2">
					...
					<div class="new3">
						<h2><?php the_title(''); ?></h2>
						<div class="new3_1">
							发布时间：<?php the_time('20y-m-d')?> 发布人：<?php the_author(); ?>
						</div>
					</div>
						<?php the_content(); ?>
					...
					<?php endwhile; endif; ?>
					<div class="new4">
					<?php if (get_next_post(true)) { next_post_link('上一篇: %link','%title',true);} else { echo '上一篇：已是最新文章';} ?><br />
					<?php if (get_previous_post(true)) { previous_post_link('下一篇: %link','%title',true);} else { echo '下一篇：已是最后文章';} ?>
					</div>
				</div>
				<div class="g3"></div>
			</div>
